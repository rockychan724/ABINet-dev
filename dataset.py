...
from utils import CharsetMapper, onehot, TokenLabelConverter
...
class ImageDatasetMGP(Dataset):
    ...
    def __init__(self,
        ...
        self.c = len(self.converter.character)
    ...
    def __getitem__(self, idx):
        ...
        length = tensor(len(text) + 2).to(dtype=torch.long)  # two for start token and end token
        char_label = self.converter.char_encode(text, case_sensitive=self.case_sensitive)
        bpe_label = self.converter.bpe_encode(text, case_sensitive=self.case_sensitive)
        wp_label = self.converter.wp_encode(text, case_sensitive=self.case_sensitive)
        ...
        if self.return_idx:
            y = [char_label, bpe_label, wp_label, length, idx_new]
        else:
            y = [char_label, bpe_label, wp_label, length]
        return image, y
